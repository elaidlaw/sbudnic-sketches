...
void loop() {
  ...
  if (Serial.available() != 0) {
    ...
    strcpy(char_array, newString.c_str());
     ...
     mydata = "SBDNIC" + mydata;
     byte sendArr[mydata.length() + 1] = {};
     int nData = mydata.length();
     char data_array[n + 1];
     strcpy(data_array, mydata.c_str());
     sprintf(sendArr, "%s", data_array);
     
     int stateSend = radio.transmit(sendArr, nData + 1);
    }
  }
