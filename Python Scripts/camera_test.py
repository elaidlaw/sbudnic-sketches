...
while True:
    try:
        packet = read_packet()
        if packet[0][0] == 'C':
            image_id = packet[0][1:3]
            packet_number = int(packet[0][4:])
            if packet[0][3] == 'S':
                images[image_id] = [None]*int(packet_number)
                image_start_times[image_id] = datetime.now().strftime("%m-%d-%Y-%H-%M-%S")
                print(f'Receiving image {image_id} with {packet_number} packets; will be saved as {image_start_times[image_id]}.bin')
            if packet[0][3] == 'D':
                ...
                images[image_id][packet_number] = packet[1]
            ...
            data = ['0', telemetry[1], telemetry[2], telemetry[4], telemetry[5], telemetry[6], telemetry[7], telemetry[8], telemetry[9]]
